import {
  Blockfrost,
  fromUnit,
  Koios,
  Lucid,
  paymentCredentialOf,
  scriptFromNative,
  UTxO,
  Validator,
  validatorToAddress,
} from "@lucid-evolution/lucid";
import { lockNFT } from "./endpoints/lockNFT.js";
import { NFTMinter } from "./endpoints/NFTMinter.js";
import { withdrawNFT } from "./endpoints/withdrawNFT.js";
import { buyNFT } from "./endpoints/buyNFT.js";
import { queryNFT } from "./endpoints/queryNFT.js";

const nftMarketPlace =
  "";

////const network = process.env.NETWORK
//console.log(network)

const user1 = await Lucid(
  new Koios("https://preprod.koios.rest/api/v1"), // Provider
  "Preprod"
);

// create an account
// get the private key from the website
// const user1Blockfrost = await Lucid
//   new Blockfrost("block_url", "private_key"), // Provider
//   "Preprod"
// );

user1.selectWallet.fromSeed(
  "mule quit evil loyal hamster finish plastic tattoo walk grace above bring swing fiction cook corn unusual coral decade poverty lake state lift shrug"
);

// console.log(await user1.wallet().getUtxos());
const user1Addr = await user1.wallet().address();
// console.log(user1Addr);
const tokenInfo = fromUnit(
  "10ec70032c24b622e11d95eb1eebb703bca6779815b57c519c7479584275726e61626c65546f6b656e" // policy_id + token_name(hex)
);

// const contract: Validator = {
//   type: "PlutusV2",
//   script: nftMarketPlace,
// };
// const contractAddr = validatorToAddress("Preprod", contract);


// const allContractUtxos = await user1.utxosAt(contractAddr);
// console.log(allContractUtxos);

// ********  mint nft ***************
// const minted = await NFTMinter({
//   lucid: user1,
//   TokenName: "BurnableToken",
//   address: user1Addr,
// });
// console.log(minted)

//**********  lock nft in contract */
// const myTx = await lockNFT({
//   lucid: user1,
//   priceOfAsset: 10_000_000n,
//   policyID: tokenInfo.policyId,
//   TokenName: tokenInfo.assetName!,
//   marketPlace: nftMarketPlace
// });

// console.log(myTx)

//****** seller withdraws nft */
// const myWithdraw =  withdrawNFT({
//   lucid: user1,
//   marketplace: nftMarketPlace
// });

// console.log(myWithdraw);


//*****  user buys nft ***** */
const buyUtxo: UTxO[] = await queryNFT(user1 ,nftMarketPlace,10_000_000n);
console.log("******************");
console.log(buyUtxo);

const myBuy = await buyNFT({
  lucid: user1,
  marketplace: nftMarketPlace,
  utxo: buyUtxo[0]
});

console.log(myBuy);
